...
const GoalsScreen = () => {
    const { user } = useAuth();
    const { formatAmount, convertAmountToPKR } = useCurrency();
    ...
    const handleAllocate = async () => {
        const amountInSelected = parseFloat(allocationModal.amount);
        if (isNaN(amountInSelected) || amountInSelected <= 0) {
            Alert.alert("Error", "Please enter a valid amount");
            return;
        }

        // Convert the input amount from selected currency back to PKR base
        const amount = convertAmountToPKR(amountInSelected);
        ...
        if (amount > remaining) {
            Alert.alert(
                "Limit Reached", 
                `You only need ${formatAmount(remaining)} to complete this goal. Would you like to add exactly that amount?`,
                [
                    { text: "Cancel", style: "cancel" },
                    { 
                        text: "Add " + formatAmount(remaining), 
        ...
        try {
            ...
            Alert.alert(
                ...
                isCompleted 
                    ...
                    : `${formatAmount(amount)} allocated to ${goal.title}`
            );
            ...
            <View style={[styles.goalCard, isCompleted && { borderColor: colors.green }]}>
                ...
                <View style={styles.progressContainer}>
                    <View style={styles.progressLabelRow}>
                        <Text style={styles.progressText}>{formatAmount(saved, 0)} / {formatAmount(target, 0)}</Text>
