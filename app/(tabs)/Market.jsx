...
export default function Market() {
  const { convertAmount, convertAmountToPKR, currencySymbol, selectedCurrency } = useCurrency();
  ...
  // Convert current budget input (in selected currency) back to PKR to fetch products correctly
  const numericBudgetInPKR = useMemo(
    () => convertAmountToPKR(parseInt(debouncedBudget) || 0),
    [debouncedBudget, convertAmountToPKR],
  );

  const { affordablePhones, loading, error } = useProductDiscovery(
    numericBudgetInPKR,
    selectedCategory,
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setRawBalance(totalIncome - totalExpense);
    });
    return unsubscribe;
  }, []);

  // Update budget field when rawBalance or selectedCurrency changes
  useEffect(() => {
    setBudget(Math.round(convertAmount(rawBalance)).toString());
  }, [rawBalance, selectedCurrency, convertAmount]);
  ...
  const handleBuy = useCallback((item) => {
    const url =
      item.productUrl ||
      `https://www.google.com/search?q=${encodeURIComponent(item.name + " price in pakistan buy online")}`;
          ...
          <View style={styles.headerSection}>
            <View style={styles.topRow}>
              ...
              >
                ...
                <Text style={styles.wealthBtnText}>Wealth</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.budgetCard}>
              ...
              <View style={styles.budgetRow}>
                <Text style={styles.budgetCurrency}>{currencySymbol.trim()}</Text>
                <TextInput
          ...
          !loading && (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyDesc}>
                Try adjusting your filters or search outside
              </Text>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  wealthBtnText: { color: "white", fontWeight: "700", fontSize: 13 },
});
