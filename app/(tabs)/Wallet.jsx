...
const Wallet = () => {
  ...
  const calculateTotalBalance = () => {
    return transactions.reduce((acc, tx) => acc + (tx.type === "income" ? (Number(tx.amount) || 0) : -(Number(tx.amount) || 0)), 0);
  };

  const calculateWalletBalance = (walletId) => {
    return transactions
      .filter(tx => tx.walletId === walletId)
      .reduce((acc, tx) => acc + (tx.type === "income" ? (Number(tx.amount) || 0) : -(Number(tx.amount) || 0)), 0);
  };
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
              try {
                ...
                await batch.commit();
                setLoading(false);
                Alert.alert("Deleted", "Wallet and all its transactions have been removed.");
              } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 60, paddingBottom: 100 }}>
        ...
        <View style={styles.balanceCard}>
          <View>
            ...
            <Text style={styles.balanceAmount}>{formatAmount(calculateTotalBalance())}</Text>
          </View>
          ...
          <View style={styles.grid}>
            {wallets.map((item) => (
              <View key={item.id} style={styles.walletCard}>
                ...
                <Text style={styles.walletBalance}>{formatAmount(calculateWalletBalance(item.id), 0)}</Text>
              </View>
            ))}
          </View>
        )}
