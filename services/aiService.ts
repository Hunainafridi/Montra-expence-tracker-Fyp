      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expense tracker parser. Return ONLY JSON object with fields: title, amount (number), type (income/expense). No markdown.",
        },
        {
          role: "user",
          content: text,
...
export const transcribeAudioWithGroq = async (
  ...
  try {
    const normalizedUri =
      audioUri && !audioUri.startsWith("file://") && audioUri.startsWith("/")
        ? `file://${audioUri}`
        : audioUri;
    ...
    try {
      ...
      console.error(
        "Groq Transcription: failed to parse response",
        responseText,
      );
      return "";
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an e-commerce scraper. Extract the product Name, Price (number only), and Image URL from the HTML snippet. Return JSON: { name, price, image }.",
        },
        {
          role: "user",
...
export const getChatResponse = async (
  ...
  try {
    ...
    const marketContext =
      context.marketProducts && context.marketProducts.length > 0
      ...
      messages: [
        {
          role: "system",
          content:
            "Categorize this expense based on the merchant name and amount. Reply with exactly one word from this list: Food, Transport, Shopping, Bills, Entertainment, Health, Education, Other.",
        },
        {
          role: "user",
...
export const detectAnomaly = async (
  newTx: { title: string; amount: number; category: string },
  history: any[],
) => {
  try {
    const prompt = `
...
export const getSpendForecasting = async (
  recentTransactions: any[],
  currentTotal: number,
) => {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a budgeting expert. Return ONLY JSON. No explanations outside the JSON block.",
        },
